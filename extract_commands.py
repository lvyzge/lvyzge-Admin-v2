...
def make_command(alias_strings, usage: str, desc: str):
	...
	normalized_aliases = [alias.strip().lower() for alias in alias_strings if alias.strip()]
	if not normalized_aliases:
		return None

	name = normalized_aliases[0]
	return {
		"name": name,
...
def build_result(filecontent: str):
...
def command_records(result):
	for section in ("commands", "patched_commands"):
		for index, command in enumerate(result.get(section, [])):
			yield section, index, command


def command_owner(section: str, index: int, command: dict):
	return f"{section}[{index}] ({command.get('name') or '<unnamed>'})"


def duplicate_canonical_names(result):
	owners = {}
	for section, index, command in command_records(result):
		name = str(command.get("name") or "").strip().lower()
		if name:
			owners.setdefault(name, []).append(command_owner(section, index, command))
	return {name: values for name, values in sorted(owners.items()) if len(values) > 1}


def duplicate_aliases(result):
	owners = {}
	for section, index, command in command_records(result):
		owner = command_owner(section, index, command)
		for alias in command.get("aliases", []):
			normalized = str(alias or "").strip().lower()
			if normalized:
				owners.setdefault(normalized, []).append(owner)
	return {alias: values for alias, values in sorted(owners.items()) if len(values) > 1}


def validation_messages(result, strict_aliases: bool = False):
	errors = []
	warnings = []

	for name, owners in duplicate_canonical_names(result).items():
		errors.append(f"duplicate canonical command name '{name}': {', '.join(owners)}")

	for alias, owners in duplicate_aliases(result).items():
		message = f"duplicate command alias '{alias}': {', '.join(owners)}"
		if strict_aliases:
...
def main():
	parser = argparse.ArgumentParser(
		description="Generate and validate lvyzge Admin command metadata"
	)
	...
	parser.add_argument(
		"--check",
		action="store_true",
		help="Do not write; fail if output is stale or canonical names are duplicated",
	)
	parser.add_argument(
		"--strict-aliases",
		action="store_true",
		help="Treat all duplicate aliases as errors instead of warnings",
	)
	...
	result = build_result(filecontent)
	errors, warnings = validation_messages(result, strict_aliases=args.strict_aliases)
	print_messages("warning", warnings)
	print_messages("error", errors)

	if args.check:
		current, read_error = load_json(output_path)
		if read_error:
			errors.append(read_error)
			print(f"error: {read_error}", file=sys.stderr)
		elif current != result:
			message = f"{output_path} is stale; run {Path(sys.argv[0]).name}"
			errors.append(message)
		...
		print(
			f"Command metadata is current: "
			f"{len(result['commands'])} commands and "
			f"{len(result['patched_commands'])} patched commands."
		)
		return 0
	...
	print(
		f"Extracted {len(result['commands'])} commands and "
		f"{len(result['patched_commands'])} patched commands."
	)
	return 0
